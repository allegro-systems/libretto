...
struct Editor: Node {
    var body: some Node {
        Stack {
            ...
            .flex(.row, gap: 6, wraps: true)

            // Editor + preview split
            Stack {
                // Textarea (mono for code editing)
                TextArea(name: "body", placeholder: "Write in Markdown\u{2026}", id: "editor-body")
                    .flex(grow: 1)
                    .size(height: 480, minWidth: 0)
                    .padding(14)
                    .background(.elevated)
                    .font(.mono, size: 13, lineHeight: 1.6, color: .text)
                    .border(width: 1, color: .border, style: .solid)
                ...
                Stack {
                    ...
                    Stack {}
                        .htmlAttribute("id", "editor-preview")
                        .size(minHeight: 480)
                        .padding(14)
                        .background(.elevated)
                        .border(width: 1, color: .border, style: .solid)
                        .font(.sans, size: 15, lineHeight: 1.7, color: .text)
                        .font(wordBreak: .breakWord)
                        .overflow(y: .auto)
                }
                .flex(.column, gap: 8)
                .flex(grow: 1)
            ...
            // SCORE-GAP: Client-side fetch, DOM manipulation for markdown preview, toolbar
            // selection wrapping, and post save/publish require raw JS.
